...
| CV-Edge | Pipeline skeleton: file source → COCO-pretrained YOLO → stub geo → MQTT | Emits schema-valid `Observation`s from the Bengaluru footage |
...
| **CV-Edge** | Camera calibration + IPM + geometry unit tests. Pose interpolation. Map matching. Distance-gated frame sampling. Uplink queue + SQLite spool. `SegmentPass` emission including `inspected_for`. |
...
> Play `bengaluru-orr-morning.mp4`. Watch a real pothole get detected, geo-referenced, uplinked,
...
| **CV-Edge** | ByteTrack integration. Unique-track counting, occupancy ratio, PCU. Pedestrian counting with `observed_area_m2`. Multi-camera policy. Privacy gate (face blur). INT8 quantisation + delta check. |
...
| **CV-Edge** | **Hardware benchmark**: Orin / Pi+Hailo / Android, filling [`docs/11`](11-hardware-benchmark.md). ANPR with per-character voting. Incident behaviour classifier. Thermal soak test. |
...
**Exit criteria:** the benchmark table in [`docs/11`](11-hardware-benchmark.md) is filled with
measured numbers. A complete incident flows from video to report. Every clause in
...
```
  CV-Perception ──ONNX file + class list──▶ CV-Edge
...
| 2. Android benchmark target | Two-target comparison instead of three | Orin vs Pi is the meaningful spread |
...
| 5. Side + cabin cameras | Front + rear only | Front does the heavy lifting; the policy is documented |
